...
if update:
	# Update the probelist from the AMP database (user has sudo rights to postgres user)
	print("Updating list of probes and endpoints...")
	command = "sudo -u postgres /usr/bin/psql ampmeta -c".split()

	# SQL returns probeid, name, location, hardware, endpoint, staticIP
	select = "COPY (SELECT site_ampname, \
					site_longname AS access, \
					site_location AS location, \
					left(site_description,position(';' in site_description)-1) AS hardware, \
					substring(site_description,'; ([^;]+);[^;]+$') AS endpoint, \
					substring(site_description,'; ([^;]+)$') AS staticIP \
					FROM site INNER JOIN member ON site.site_ampname = member.member_ampname "
	close = ") TO STDOUT WITH CSV FORCE QUOTE access,location,hardware,endpoint,staticIP"

	# Probe IDs
	sql = select + "WHERE member_meshname='probes' ORDER BY member_ampname" + close
	probeList = subprocess.check_output(command + [sql]).decode('utf-8')
	# Endpoint FQDNs
	sql = select + "WHERE member_meshname LIKE 'endpoints%' ORDER BY member_meshname,member_ampname" + close
	endpointList = subprocess.check_output(command + [sql]).decode('utf-8')
	...
	with open(ansibleInventory, 'w') as f:
		...
		f.write("[probes]\n")
		for line in probeList.splitlines():
			elements = line.split(',')
			f.write(elements[0] + " access=" + elements[1] + " location=" + elements[2] + " hardware=" + elements[3] +
					" endpoint=" + elements[4] + " staticIP=" + elements[5] + "\n")
		f.write("[endpoints]\n")
		for line in endpointList.splitlines():
			elements = line.split(',')
			f.write(elements[0] + " access=" + elements[1] + " location=" + elements[2] + " hardware=" + elements[3] +
					" endpoint=" + elements[4] + " staticIP=" + elements[5] + "\n")
		for groupname, groupinfo in ansibleGroups.items():
			...
			if type(groupinfo) == list:
				...
				vars = None
			f.write("[" + groupname + "]\n")
			for line in (probeList + endpointList).splitlines():
				elements = line.split(',')
				if re.search(str(match), line):
					f.write(elements[0] + "\n")
			if vars:
